...
import org.openpnp.machine.reference.feeder.wizards.VisionCapFeederConfigurationWizard;
 ...
 */
public class VisionCapFeeder extends ReferenceFeeder {
    ...
    @Element(required = false)
    private Length detectX = new Length(8, LengthUnit.Millimeters);

    @Element(required = false)
    private Length detectY = new Length(8, LengthUnit.Millimeters);

    @Element(required = false)
    private Length detectWidth = new Length(8, LengthUnit.Millimeters);

    @Element(required = false)
    private Length detectHight = new Length(8, LengthUnit.Millimeters);
    ...
    public Length getDetectX() {
        return detectX;
    }

    public void setDetectX(Length detectX) {
        Logger.info(String.format("detectX:%f",detectX.getValue()));
        this.detectX = detectX;
    }

    public Length getDetectY() {
        return detectY;
    }

    public void setDetectY(Length detectY) {
        this.detectY = detectY;
    }

    public Length getDetectWidth() {
        return detectWidth;
    }

    public void setDetectWidth(Length detectWidth) {
        this.detectWidth = detectWidth;
    }

    public Length getDetectHight() {
        return detectHight;
    }

    public void setDetectHight(Length detectHight) {
        this.detectHight = detectHight;
    }
    ...
    public VisionCapFeeder() {
        // Listen to the machine become unhomed to invalidate feeder calibration.
        // Note that home() first switches the machine isHomed() state off, then on
        // again,
    ...
    public Location getPickLocation() throws Exception {
        ...
        // Determine the angle that the tape is at. For historical reasons, our local
        // coordinates are
    ...
    public Location[] getIdealLineLocations() {
        ...
        if (visionLocationReference == null) {
            ...
            // Apply the same offset to the configured last hole, so that the relative
            // position (ie tape angle) is unchanged
        ...
        double d2 = effectiveReferenceHoleLocation.getLinearLengthTo(visionLocation)
                .convertToUnits(LengthUnit.Millimeters).getValue();
        if (d2 > d1 * 0.9) { // factor to bias towards using vision location if distances are approximately
                             // equal
    ...
    public void takeBackPart(Nozzle nozzle) throws Exception {
        // first check if we can and want to take back this part (should be always be
        // checked before calling, but to be sure)
        if (nozzle.getPart() == null) {
            throw new UnsupportedOperationException("No part loaded that could be taken back.");
        }
        if (!nozzle.getPart().equals(getPart())) {
            throw new UnsupportedOperationException("Feeder: " + getName() + " - Can not take back "
                    + nozzle.getPart().getName() + " this feeder only supports " + getPart().getName());
        }
        if (!canTakeBackPart()) {
            throw new UnsupportedOperationException(
                    "Feeder: " + getName() + " - Currently no free slot. Can not take back the part.");
        }
    ...
    public void setTapeWidth(Length tapeWidth) {
        Logger.info(String.format("tapeWidth:%f",tapeWidth.getValue()));
    ...
    void commit() {
        if (standardEia481 == null) {
            // If loaded from old configuration, we must migrate it to the EIA-481-C
            // industry
    ...
    public Wizard getConfigurationWizard() {
        return new VisionCapFeederConfigurationWizard(this);
    }
    ...
    private static CvPipeline createDefaultPipeline() {
        try {
            String xml = IOUtils.toString(VisionCapFeeder.class
                    .getResource("VisionCapFeeder-DefaultPipeline.xml"));
